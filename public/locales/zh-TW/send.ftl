...
sendYourFilesLink = 試用 Fubukifox Send
...
notSupportedOutdatedDetail = 很可惜，此版本的 Firefox 不支援 Fubukifox Send 所需的 Web 技術。請更新瀏覽器後再使用。
updateFirefox = 更新 Firefox
deletePopupCancel = 取消
deleteButtonHover = 刪除
passwordTryAgain = 密碼不正確，請再試一次。
javascriptRequired = Fubukifox Send 需要開啟 JavaScript 功能
whyJavascript = 為什麼 Fubukifox Send 需要 JavaScript 才能使用？
